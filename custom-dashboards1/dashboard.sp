dashboard "custom_dashboard" {
  ...
  container {
    ...
    chart {
      ...
      query = query.simulate_ec2_metric_data
      width = 12
      type = "line"
    }
  }
...
#List of unassociated security groups
...
#RDS DB Instance deletion protection status
...
#Application Load Balancer count
...
#Elasticache with less than 2 AZ
...
#Elasticache with automated backup enabled
...
query "simulate_ec2_metric_data" {
  sql = <<-EOQ
    -- Simulate EC2 metric data for CPUUtilization
    with instances as (
    ...
    select
      instance_id,
      'CPUUtilization' as metric_name,
      'AWS/EC2' as namespace,
      generate_series(now() - interval '1 day', now(), '1 hour'::interval) as period_start,
      random() * 100 as average
    from
      instances
  EOQ
}
