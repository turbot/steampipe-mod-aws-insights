dashboard "rds_db_instance_snapshot_dashboard" {
  ...
  container {
    ...
    card {
      ...
      width = 2
      query = query.rds_db_instance_snapshot_cost_mtd
    }
  ...
  container {
    ...
    table {
      width = 6
      title = "Forecast"
      query = query.rds_db_instance_snapshot_monthly_forecast_table
    }
    ...
    chart {
      ...
      query = query.rds_db_instance_snapshot_cost_per_month
    }
...
query "rds_db_instance_snapshot_cost_mtd" {
  sql = <<-EOQ
    select
      'Cost - MTD' as label,
...
query "rds_db_instance_snapshot_monthly_forecast_table" {
  sql = <<-EOQ
...
query "rds_db_instance_snapshot_cost_per_month" {
  sql = <<-EOQ
    select
