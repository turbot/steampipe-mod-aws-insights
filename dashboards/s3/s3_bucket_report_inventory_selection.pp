dashboard "s3_bucket_inventory_report_selection" {

  title         = "AWS S3 Bucket Inventory Report Selection"
  ...
  table {
    ...
    query = query.s3_bucket_inventory_table_selection
  }

}

query "s3_bucket_inventory_table_selection" {
  sql = <<-EOQ
    select
      b.title as "Name",
