...
query "aws_nlb_graph_relationships" {
  sql = <<-EOQ
    ...
    select
      ...
      jsonb_build_object( 'ARN', arn, 'Account ID', account_id, 'Region', region, 'Security Groups', nlb.security_groups ) as properties 
    from
      nlb
    ...
    select
      ...
      'logs to' as title,
      'uses' as category,
      jsonb_build_object( 'Name', nlb.name, 'ARN', nlb.arn, 'Account ID', nlb.account_id, 'Region', nlb.region, 'Logs to', attributes ->> 'Value' ) as properties 
    from
      aws_s3_bucket buckets,
      nlb,
      jsonb_array_elements(nlb.load_balancer_attributes) attributes 
